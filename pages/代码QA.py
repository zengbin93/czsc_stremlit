...
@st.cache_resource
def initialize_chain_model(model_name='qwen-max-1201', api_key=None, api_base=None):
    from langchain.vectorstores import DeepLake
    from langchain.chat_models import ChatOpenAI
...
def show_kbqa():
    ...
    3. https://www.langchain.com.cn/use_cases/code/code-analysis-deeplake
    ...
    model_name = c3.selectbox(label="请选择模型", options=model_names, index=0)
    if c4.button("清空历史消息"):
        st.session_state.messages = []
    if c5.button("微信捐赠支持，接GPT4"):
        st.success("感谢支持，请加微信：zengbin93，备注：捐赠")
    st.divider()
    if model_name.startswith('gpt-4') and st.experimental_user.email not in allow_gpt4_users:
...
def show_chain_kbqa():
    ...
    api_key = None if api_key == "使用默认值" else api_key
    api_base = None if api_base == "使用默认值" else api_base
    qa = initialize_chain_model(model_name=model_name, api_key=api_key, api_base=api_base)
    ...
    if prompt := st.chat_input("What is up?"):
        ...
        with st.chat_message("assistant"):
            answers = [x['content'] for x in st.session_state.messages if x['role'] == 'assistant']
            questions = [x['content'] for x in st.session_state.messages if x['role'] == 'user']
            chat_history = [(x, y) for x, y in zip(questions, answers)]
            response = qa({"question": prompt, "chat_history": chat_history})['answer']
